...
export const env = {
  port: readListenPort("RADAR_SCORE_PORT", 8013),
  host: process.env.HOST ?? "0.0.0.0",
  mongoUri: process.env.MONGODB_URI?.trim() || undefined,
  mongoDb: process.env.MONGODB_DB?.trim() || "project_radar_score",
  mongoScoresCollection:
    process.env.MONGODB_SCORES_COLLECTION?.trim() || "scores",
} as const;
