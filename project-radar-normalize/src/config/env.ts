...
export const env = {
  port: readListenPort("RADAR_NORMALIZE_PORT", 8012),
  host: process.env.HOST ?? "0.0.0.0",
  mongoUri: process.env.MONGODB_URI?.trim() || undefined,
  mongoDb: process.env.MONGODB_DB?.trim() || "project_radar_normalize",
  mongoProjectsCollection:
    process.env.MONGODB_PROJECTS_COLLECTION?.trim() || "projects",
  scoreUrl:
    process.env.RADAR_SCORE_URL?.trim() ||
