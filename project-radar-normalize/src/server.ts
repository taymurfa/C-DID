...
export async function buildServer() {
  ...
  await app.register(swagger, {
    openapi: {
      info: {
        title: "Project Radar - Normalize (R2)",
        description: "Entity resolution across ERCOT / PUCT / TCEQ records.",
        version: "0.1.0",
      },
      servers: [{ url: `http://localhost:${env.port}` }],
      tags: [
        { name: "radar", description: "Normalize + ER" },
  ...
  app.log.info(
    {
      ...
      scoreUrl: env.scoreUrl ?? "none",
    },
    "project-radar-normalize configuration",
  );
