...
/**
 * Scoring brain: Agent 2 POST /qualify. Falls back to embedded Person 2 when
 * intelligence is unreachable so judging still works offline.
 */
async function qualifyViaIntelligence(
  ...
  try {
    ...
    const mapped = mapIntelligenceToQualifyResponse(payload, source, mode);
    if (minTier) {
