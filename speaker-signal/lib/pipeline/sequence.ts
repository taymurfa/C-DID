...
export type SequenceLead = Pick<
  Speaker,
  "id" | "name" | "title" | "company" | "conference" | "session" | "evidence"
> & {
  topics?: string[];
};

export type SequenceConferenceInput = {
  name: string | null;
  startDate: string;
  endDate?: string | null;
 ...
 * Status vs `now`: past email touches → Sent; Event → Opportunity;
 * next upcoming email → Scheduled; later emails → Planned.
 */
...
function evidenceLabels(lead: SequenceLead): string[] {
  const fromEvidence = lead.evidence.map((e) => e.label || e.sourceUrl).filter(Boolean);
  const extras: string[] = [];
...
export async function draftSequenceEmails(
  lead: SequenceLead | QualifiedLead,
  conference: SequenceConferenceInput,
  steps: SequenceStep[],
  ...
  const userPayload = {
    lead: {
      ...
      topics: "topics" in lead ? lead.topics ?? [] : [],
