      ...
      <section className="workspace" id="overview">
        <header className="command-bar">
          ...
          <button className="mode-button" onClick={() => data.setDemoMode((value) => !value)} aria-pressed={data.demoMode}>
            <Database size={16} />
            {data.demoMode ? "Demo data" : "Live Firecrawl"}
            <ChevronDown size={15} />
          </button>
        </header>
        ...
        {data.notice || data.error ? (
          <div className={`analysis-notice ${data.error ? "notice-error" : ""}`} role="status">
            ...
            {data.notice && (
              <small>
                {data.notice.mode.toUpperCase()} · {data.notice.speakersIngested} ingested · {data.notice.qualified} qualified
              </small>
            )}
        ...
        <section className="metric-strip" aria-label="Key metrics">
          ...
          <Metric
            ...
            note={data.stats ? `${data.stats.afterDedupe} after dedupe` : "Seed desk data"}
            positive
          />
          <Metric
            icon={Send}
...
function FunnelPanel({ funnel }: { funnel: ReturnType<typeof useSignalData>["funnel"] }) {
  ...
  return (
    <section className="panel funnel-panel" id="funnel">
      ...
      <div className="funnel-meta"><span>Live lead statuses</span><span><i /> In-memory</span></div>
