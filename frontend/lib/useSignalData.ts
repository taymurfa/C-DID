...
function conferencePayloadFrom(
  ...
  // Live ingest often omits dates — still allow sequence generation.
...
export function useSignalData() {
  ...
  // Poll Agent 1/2/3 health for the System / Agent Runs panel.
  useEffect(() => {
    let cancelled = false;
  ...
  // Bootstrap desk from /api/qualify (demo) + /api/funnel so UI is API-backed.
  useEffect(() => {
    let cancelled = false;
  ...
  const advanceStatus = useCallback(
    (leadId: string) => {
      setStatuses((prev) => {
        ...
        // Optimistic local funnel until API refresh lands.
        setApiFunnel(null);
        setFunnelSource("local");
